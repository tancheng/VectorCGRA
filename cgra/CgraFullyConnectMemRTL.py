...
class CgraRTL(Component):
  ...
  def construct(s, DataType, PredicateType, CtrlPktType, CtrlSignalType,
    ...
    # Connects both first col's tiles and last row's tiles to data mem.
    s.data_mem_num_rd_tiles = width + height - 1
    s.data_mem_num_wr_tiles = width + height - 1
    ...
    for i in range(s.num_tiles):
      ...
      if (i % width == 0) or (i >= s.num_tiles - width + 1):
        ...
        s.tile[i].to_mem_wdata   //= s.data_mem.recv_wdata[s.data_mem_rd_wr_idx]
        s.data_mem_rd_wr_idx += 1
      else:
