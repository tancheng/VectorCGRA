"""
==========================================================================
CgraRTL_axpy_test_from_yaml.py
==========================================================================
Test cases for CGRA with crossbar-based data memory and ring-based control
memory of each tile, using axpy.yaml compiled kernel.
...
# Preload 16 data values at addresses 0~15 with values 10~25.
# Both GEPs in axpy.yaml have no explicit base address (implicitly 0),
# so both LOADs access data[i]. The kernel computes:
#   for i in [0, 16):
#     result = 3 * data[i] + data[i]   (= 4 * data[i])
#     store(result, addr[i])
preload_data = [
    [
...
# AXPY kernel (from compiled axpy.yaml):
#
# Kernel semantics (constants baked into axpy.yaml):
#   i = 0          (GRANT_ONCE #0)
#   while (i != 16):   (ICMP_EQ #16)
#     addr = 0 + i     (GEP, implicit base 0)
#     data[addr] = 3 * data[addr] + data[addr]
#     i += 1           (ADD #1)
#   RETURN_VOID        (signals completion, no return value)
#
# With data[k] = 10 + k for k in [0, 16):
#   After execution, data[k] = 4 * (10 + k) for k in [0, 16)
#   RETURN_VOID sends CMD_COMPLETE with data = 0


def sim_axpy(cmdline_opts, mem_access_is_combinational):
  src_ctrl_pkt = []
  complete_signal_sink_out = []
  src_query_pkt = []

  # kernel specific parameters (matching axpy.yaml constants).
  kLoopLowerBound = 0         # GRANT_ONCE #0
  kLoopIncrement = 1          # ADD #1
  kLoopUpperBound = 16        # ICMP_EQ #16
  kCoefficient = 3            # MUL #3
  ...
  trace_file = os.path.join(trace_dir, 'trace_axpy_4x4_Mesh.jsonl')
...
def test_homogeneous_4x4_axpy_combinational_mem_access(cmdline_opts):
  sim_axpy(cmdline_opts, mem_access_is_combinational = True)
