"""
==========================================================================
CgraRTL_relu4x4_test_from_yaml.py
==========================================================================
Test cases for CGRA with crossbar-based data memory and ring-based control
memory of each tile, using relu.yaml compiled kernel.

ReLU kernel semantics (from relu.yaml):
  for i = 0 to 31:
    val = data[0 + i]          // GEP #0 + LOAD
    cmp = (val >= 0)           // ICMP_SGE with const #0
    result = cmp ? val : 0     // SEL
    data[0 + i] = result       // GEP #0 + STORE
...
# Preload 32 data values at addresses 0~31.
# Use values: -16, -15, -14, ..., -1, 0, 1, 2, ..., 15
# After ReLU: 0, 0, 0, ..., 0, 0, 1, 2, ..., 15
preload_data = [
    [
        IntraCgraPktType(0, 0, payload = CgraPayloadType(CMD_STORE_REQUEST, data = DataType(to_uint32(-16 + i), 1), data_addr = i))
        for i in range(32)
    ]
]

# ReLU kernel (from compiled relu.yaml):
#
# Kernel semantics (constants baked into relu.yaml):
#   i = 0          (GRANT_ONCE #0 at core 11)
#   val = 0        (GRANT_ONCE #0 at core 8 — initial PHI value, not used beyond first iteration)
#   while (i != 32):   (ICMP_EQ #32 at core 10)
#     addr = 0 + i     (GEP #0, i at cores 6 and 11)
#     val = data[addr]  (LOAD at core 3)
#     cmp = (val >= 0)  (ICMP_SGE with #0 at core 3)
#     result = cmp ? val : 0  (SEL at core 1)
#     data[addr] = result     (STORE at core 1)
#     i += 1           (ADD #1 at core 10)
#   return void        (RETURN_VOID at core 9)
#
# With data[k] = -16 + k for k in [0, 32):
#   After ReLU:
#     data[0..15] = 0 (negatives clamped to 0)
#     data[16] = 0
#     data[17..31] = 1..15


def sim_relu_return(cmdline_opts, mem_access_is_combinational):
  src_ctrl_pkt = []
  complete_signal_sink_out = []
  src_query_pkt = []

  # kernel specific parameters (matching relu.yaml constants).
  ...
  trace_file = os.path.join(trace_dir, 'trace_relu4x4_4x4_Mesh.jsonl')
...
def test_homogeneous_4x4_relu_combinational_mem_access_return(cmdline_opts):
  sim_relu_return(cmdline_opts, mem_access_is_combinational = True)
