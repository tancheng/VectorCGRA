...
class RegisterBankRTL(Component):
  ...
  def construct(s, DataType, CtrlType, reg_bank_id, num_registers = 4,
    ...
    def update_token_status():
      ...
      if enable_token_discipline:
        s.outport_wr_rdy @= ~s.pending_valid & \
            (~s.write_token_valid | same_register_fu_feedback)
      else:
        s.outport_wr_rdy @= 1
    ...
    def access_registers():
      ...
      # empty, including the cycle in which the current ctrl step consumes
      # the old token. Pending promotion has priority over a new write.
      if ~s.reset & s.pending_valid & \
        ...
        if s.inport_valid[write_reg_from - 1] & s.outport_wr_rdy:
          ...
          # When the old token is consumed in this same cycle, replace it
          # atomically. Otherwise retain the result in the pending slot.
          if enable_token_discipline & s.write_token_valid & \
             ~write_target_will_clear:
            s.pending_capture @= 1
          else:
