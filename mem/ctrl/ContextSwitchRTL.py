...
class ContextSwitchRTL(Component):
  ...
  def construct(s, data_nbits, ctrl_addr_nbits):
    ...
    # through the port 'phi_unique_addr' to the register 'phi_unique_addr_reg'.
    ...
    def update_msg():
      ...
      s.is_executing_phi @= ((s.recv_opt == OPT_PHI_CONST) and (s.phi_unique_addr_reg == s.ctrl_mem_rd_addr))
    ...
    def update_regs():
      ...
      if (s.recv_cmd_vld & (s.recv_cmd == CMD_PHI_ADDR)):
        s.phi_unique_addr_reg <<= s.phi_unique_addr
      else:
        s.phi_unique_addr_reg <<= s.phi_unique_addr_reg
  ...
  def line_trace(s):
    ...
    phi_unique_addr_str = f'|| phi_unique_addr: {s.phi_unique_addr} '
    ...
    register_content_str = f'|| progress_reg: {s.progress_reg} | status_reg: {s.status_reg}i | phi_unique_addr_reg: {s.phi_unique_addr_reg} '
    condition_str = f'|| condition: {s.progress_is_null}{s.is_pausing}{s.is_executing_phi} '
    return recv_cmd_str + recv_opt_str + progress_in_str + progress_out_str + phi_unique_addr_str + ctrl_mem_rd_addr_str + overwrite_fu_output_predicate_str + register_content_str + condition_str
