...
def drive_defaults(dut, DataAddrType, DataType, NocPktType, num_rd_tiles, num_wr_tiles):
  ...
  dut.recv_spm_wr_req.val @= 0
  dut.recv_spm_wr_req.msg.addr @= DmaSpmAddrType(0)
  dut.recv_spm_wr_req.msg.data @= 0
  dut.recv_spm_wr_req.msg.mask @= 0
  dut.recv_spm_rd_req.val @= 0
  dut.recv_spm_rd_req.msg.addr @= DmaSpmAddrType(0)
  dut.send_spm_rd_resp.rdy @= 1
...
def test_dma_ports_write_then_read():
  ...
  DmaSpmAddrType = mk_dma_cmd().get_field_type(kAttrSpmAddr)
  dut.recv_spm_wr_req.val @= 1
  dut.recv_spm_wr_req.msg.addr @= DmaSpmAddrType(3)
  dut.recv_spm_wr_req.msg.data @= 0xaaaabbbb
  dut.recv_spm_wr_req.msg.mask @= 0xf
  dut.sim_eval_combinational()
  assert dut.recv_spm_wr_req.rdy
  dut.sim_tick()
  dut.recv_spm_wr_req.val @= 0

  dut.recv_spm_rd_req.val @= 1
  dut.recv_spm_rd_req.msg.addr @= DmaSpmAddrType(3)

  seen_response = False
  for _ in range(10):
    dut.sim_eval_combinational()
    if dut.recv_spm_rd_req.val & dut.recv_spm_rd_req.rdy:
      dut.recv_spm_rd_req.val @= 0
    if dut.send_spm_rd_resp.val:
      assert int(dut.send_spm_rd_resp.msg.data) == 0xaaaabbbb
      seen_response = True
      break
    dut.sim_tick()
