  ...
  def construct(s,
    ...
    def assemble_xbar_pkt():
      ...
      if has_dma_ports:
        ...
        recv_raddr_from_dma = trunc(s.recv_from_ctrl_spm_rd_req.msg.addr, AddrType)
        ...
        recv_waddr_from_dma = trunc(s.recv_from_ctrl_spm_wr_req.msg.addr, AddrType)
        ...
        s.wr_pkt[dma_wr_idx] @= MemWritePktType(dma_wr_idx,                 # src
                                                ...
                                                DataType(zext(s.recv_from_ctrl_spm_wr_req.msg.data, PayloadType), 1, 0, 0),
                                                0,                          # src_cgra
                                                0,                          # src_tile
                                                0)                          # remote_src_port
    ...
    def update_all():
      ...
      s.recv_from_ctrl_spm_wr_req.rdy          @= 0
      s.recv_from_ctrl_spm_rd_req.rdy           @= 0
      s.send_to_ctrl_spm_rd_resp.val      @= 0
      s.send_to_ctrl_spm_rd_resp.msg      @= DmaSpmReadRespType(DmaSpmDataType(0))
      ...
      if has_dma_ports:
        ...
        dma_rd_idx = XbarInRdType(num_xbar_in_rd_ports - 1)
        s.read_crossbar.recv[dma_rd_idx].val @= s.recv_from_ctrl_spm_rd_req.val
        s.read_crossbar.recv[dma_rd_idx].msg @= s.rd_pkt[dma_rd_idx]
        s.recv_from_ctrl_spm_rd_req.rdy @= s.read_crossbar.recv[dma_rd_idx].rdy
      ...
      if has_dma_ports:
        ...
        # between `dma_wr_idx` and `num_xbar_in_wr_ports`.
        dma_wr_idx = XbarInWrType(num_xbar_in_wr_ports - 1)
        s.write_crossbar.recv[dma_wr_idx].val @= s.recv_from_ctrl_spm_wr_req.val
        s.write_crossbar.recv[dma_wr_idx].msg @= s.wr_pkt[dma_wr_idx]
        s.recv_from_ctrl_spm_wr_req.rdy @= s.write_crossbar.recv[dma_wr_idx].rdy
      ...
      for i in range(num_xbar_in_rd_ports):
        if i < num_rd_tiles:
          ...
          s.send_to_ctrl_spm_rd_resp.msg      @= DmaSpmReadRespType(
            trunc(s.response_crossbar.send[i].msg.data.payload, DmaSpmDataType))
          s.send_to_ctrl_spm_rd_resp.val      @= s.response_crossbar.send[i].val
          s.response_crossbar.send[i].rdy @= s.send_to_ctrl_spm_rd_resp.rdy
