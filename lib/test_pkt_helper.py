...
def test_generate_cpu_pkt_from_json_generates_thread_span_ld_st_counts(tmp_path):
    ...
    assert int(cpu_metadata_pkts[0].tokenizer_cfg.token_route_sink_enable[0]) == 0b111
    assert ld_pkts[0] == [5] * 80
    assert st_pkts[0] == 4
    assert expected_cpu_pkts == [1]


def test_generate_cpu_pkt_from_json_preserves_full_512_thread_span(tmp_path):
    json_path = tmp_path / "mapped_512.json"
    json_path.write_text(json.dumps({
        "cgra_def": {
            "num_tiles": 2,
            "num_tile_rows": 1,
        ...
        "cfg_0": {
            "metadata": {
                ...
                "out_pred_regs_val": [0],
                "tokenizer": {
                    "token_route_sink_enable": [[1]],
                    "token_route_delay_to_sink": [0]
                },
                "cfg_id": 0,
                "br_id": 0,
                "thread_count_min": 0,
                "thread_count_max": 512,
                "thread_count": 512,
                "start_cfg": 1,
                "end_cfg": 1,
                "branch_en": 0,
    ...
    _, cpu_metadata_pkts, _, _, _, _ = generateCPUPktFromJSON(str(json_path))

    assert len(cpu_metadata_pkts) == 2
    assert int(cpu_metadata_pkts[0].thread_count_min) == 0
    assert int(cpu_metadata_pkts[0].thread_count_max) == 512
