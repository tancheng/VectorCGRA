...
def mk_ctrl(num_fu_inports = 4,
  ...
  def str_func(s):
    ...
    out_str += '|(read_reg_retain)' + '-'.join(
        str(int(s.read_reg_retain[i])) for i in range(num_fu_inports))
  ...
  # Keep a read value live for later ctrl steps; zero consumes it as before.
