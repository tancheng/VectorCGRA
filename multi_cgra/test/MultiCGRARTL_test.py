...
from ..MultiCGRARTL import MultiCGRARTL
...
class TestHarness(Component):
  ...
  def construct(s, DUT, FunctionUnit, FuList, DataType, PredicateType,
                CtrlType, num_cgras, width, height, ctrl_mem_size,
    ...
    s.src_opt = [[TestSrcRTL(CtrlType, src_opt[i])
                  for i in range(s.num_tiles)] for _ in range(
                      s.num_cgras)]
    s.ctrl_waddr = [[TestSrcRTL(AddrType, ctrl_waddr[i])
                     for i in range(s.num_tiles)] for _ in range(
                         s.num_cgras)]

    s.dut = DUT(DataType, PredicateType, CtrlType, num_cgras, width,
                height, ctrl_mem_size, data_mem_size, len(src_opt[0]),
