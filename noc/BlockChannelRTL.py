...
# BlockChannelRTL.py
#=========================================================================
# RTL channel module for testing the delayed data receiving. Basically, it
# set the recv.rdy signal after a few cycles.
#
# Author : Cheng Tan
#   Date : Aug 26, 2023
...
class BlockChannelRTL( Component ):
  def construct(s, DataType, delay = 5 ):
    ...
    @update
    def process():
      s.recv.rdy          @= s.bypass_q.enq_rdy & (s.timing >= delay)
      ...
      if ~s.bypass_q.deq_msg.bypass:
        ...
        s.send.msg.delay  @= s.queue.deq_msg.delay | ~s.queue.deq_rdy
