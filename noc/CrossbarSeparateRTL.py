...
class CrossbarSeparateRTL( Component ):
  ...
  def construct(s, DataType, PredicateType, CtrlType,
    ...
    print("[cheng] num_inports: ", num_inports)
    print("[cheng] num_outports: ", num_outports)
    ...
    # Interfaces outports of the specific crossbar.
    # s.xbar_outport = [RecvIfcRTL(InType) for _ in range(num_outports)] # s.recv_opt.msg.routing_xbar_outport[i]
    ...
    def update_signal():
      ...
      if s.recv_opt.msg.ctrl != OPT_START:
        for i in range( num_inports ):
          ...
          if s.recv_opt.msg.routing_predicate_in[i] & s.recv_data[i].en:
        ...
        print("[cheng] crossbar_outport: ", str(s.crossbar_outport))

        for i in range( num_outports ):
          # s.in_dir[i]  @= s.recv_opt.msg.routing_xbar_outport[i]
          s.in_dir[i]  @= s.crossbar_outport[i]
          print("[cheng] len(s.recv_data): ", len(s.recv_data), "; s.in_dir_local[", i, "]: ", s.in_dir_local[i], "; s.in_dir[", i, "]: ", s.in_dir[i], "; num_inports: ", num_inports, "; s.send_data[", i, "].rdy: ", s.send_data[i].rdy)
          if (s.in_dir[i] > 0) & s.send_data[i].rdy:
            s.in_dir_local[i] @= s.in_dir[i] - 1

            print("[cheng] step 1; len(s.recv_data): ", len(s.recv_data), "; s.in_dir_local[", i, "]: ", s.in_dir_local[i], "; s.in_dir[", i, "]: ", s.in_dir[i], "; num_inports: ", num_inports)
            ...
            if s.send_data[i].en & s.recv_data[s.in_dir_local[i]].rdy:
              ...
              # FIXME: Cleanup the commented logic.
              ...
              # loop.
              # print("[cheng] s.in_dir_local[", i, "]: ", s.in_dir_local[i], "; s.bypass_point: ", s.bypass_point)
