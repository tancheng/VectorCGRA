...
additional count_reset port to reset its counter.
...
class ChannelResetRTL( Component ):
  ...
  def construct(s, PacketType, QueueType=NormalQueueRTL, latency=0 ):
    ...
    if s.QueueType != None and s.latency > 0:
      ...
      s.queues = [ s.QueueType( PacketType, s.num_entries, reset_for_count=True )
      ...
      for i in range(s.latency):
        s.queues[i].count_reset //= s.count_reset
